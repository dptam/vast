...
ssh -o StrictHostKeyChecking=no -p $ssh_port root@$ssh_addr -L 8080:localhost:8080 "tmux new -d 'export PATH=/opt/conda/bin:$PATH; cd $git_proj; source bin/setup.sh; bash bin/train.sh $config_file &> output'"
